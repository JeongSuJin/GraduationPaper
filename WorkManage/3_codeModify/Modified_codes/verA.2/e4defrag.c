 ...
 *
 * @blk_limit:		if an extent has blks more than blk_limits,
 *			   the extent will not be gotten by get_file_extents.
 * @fd:			defrag target file's descriptor.
 * @ext_list_head:	the head of the extent list.
 */
static int get_file_extents(int fd, unsigned long long blk_limit, struct fiemap_extent_list **ext_list_head)
{
	__u32	i;
	int	ret;
	...
	do {
		...
		for (i = 0; i < fiemap_buf->fm_mapped_extents; i++) {
			...
			/* MODIFIED CODE by jsj*/
			unsigned long long blks_num = ext_buf[i].fe_length
							/ block_size;
			if(blks_num <= blk_limit){
				ext_list = NULL;
				ext_list = malloc(sizeof(struct fiemap_extent_list));
				if (ext_list == NULL)
					goto out;
			...
			struct FTW *ftwbuf EXT2FS_ATTR((unused)))
{
	...
	ret = get_file_extents(fd,1,&physical_list_head);
	if (ret < 0) {
		if (mode_flag & DETAIL) {
			PRINT_FILE_NAME(file);
			...
			struct FTW *ftwbuf EXT2FS_ATTR((unused)))
{
	...
	/* Get file's extents */
	ret = get_file_extents(fd,1, &orig_list_physical);
	if (ret < 0) {
		if (mode_flag & DETAIL) {
			PRINT_FILE_NAME(file);
	...
	ret = get_file_extents(donor_fd,1, &donor_list_physical);
	if (ret < 0) {
		if (mode_flag & DETAIL) {
			PRINT_FILE_NAME(file);
			PRINT_ERR_MSG_WITH_ERRNO(NGMSG_FILE_EXTENT);
